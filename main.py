...
async def play_next(entry, vc):
    if entry != None:
        ...
        vc.play(source=source, after=partial(func=_handle_queue, loop=vc.loop, guild_id=guild_id, voice_client=vc))
    ...
    async def music(ctx, song):
        ...
        with youtube_dl.YoutubeDL(ydl_opts) as ydl:
            ...
            if current_voice_client.is_playing():
                ...
                current_voice_client.play(source, after=partial(func=_handle_queue, loop=current_voice_client.loop, guild_id=ctx.guild.id, voice_client=current_voice_client))
